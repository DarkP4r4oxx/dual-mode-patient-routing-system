...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
      <div className="absolute top-1/4 left-1/4 w-[40rem] h-[40rem] bg-blue-500 rounded-full mix-blend-multiply filter blur-[128px] opacity-10 pointer-events-none" />
      ...
      <div className="w-full max-w-md z-10 space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-bold text-blue-600 bg-blue-50 border border-blue-100 uppercase tracking-widest">
            <ShieldCheck size={14} /> Patient Portal
          </div>
        </div>

        <div className="bg-white border border-slate-100 shadow-xl shadow-slate-200/50 rounded-3xl p-8 space-y-6">
          {/* Tab toggle */}
          <div className="flex rounded-full p-1 bg-slate-100 border border-slate-200/60 shadow-inner">
            {['login', 'register'].map(m => (
              <button key={m} onClick={() => { setMode(m); setError(''); }}
                className={`flex-1 py-2.5 rounded-full text-xs font-bold transition-all uppercase tracking-widest ${mode === m ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500 hover:text-slate-700'}`}>
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center">
              <span className="px-4 text-[10px] font-bold uppercase tracking-widest text-slate-400 bg-white">or continue with</span>
            </div>
          </div>

          <button onClick={handleGoogle} disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-slate-50 border border-slate-200 font-bold py-3.5 rounded-xl transition-all text-sm hover:bg-white hover:shadow-sm text-slate-700">
          ...
          <p className="text-center text-xs font-semibold text-slate-500 pt-2">
            Are you a doctor?{' '}
            <Link to="/doctor-login" className="text-blue-600 hover:text-blue-700 font-bold ml-1">Doctor Login →</Link>
          </p>
        </div>
      </div>
    </div>
